...
class JinjaEnvVar(jinja2.ext.Extension):
  ...
  def env_var(env_var: str, default: Optional[str] = None) -> str:
    ...
    if result is None:
       ...
       print("Could not find environmental variable %s and no default value was provided" % env_var)
       sys.exit(127)
...
def deploy_command(config):
  ...
  if not config['snowflake-account'] or not config['snowflake-user'] or not config['snowflake-role'] or not config['snowflake-warehouse']:
    ...
    print("Missing config values. The following config values are required: snowflake-account, snowflake-user, snowflake-role, snowflake-warehouse")
    sys.exit(127)
  ...
  if "SNOWFLAKE_PASSWORD" not in os.environ and "SNOWSQL_PWD" not in os.environ and "SNOWFLAKE_PRIVATE_KEY_PATH" not in os.environ:
    ...
    print("Missing environment variable(s). SNOWFLAKE_PASSWORD must be defined for password authentication. SNOWFLAKE_PRIVATE_KEY_PATH and (optional) SNOWFLAKE_PRIVATE_KEY_PASSPHRASE must be defined for private key authentication.")
    sys.exit(127)
  ...
  if config['ignore-errors']:
    print("Running in ignore-errors mode")
  if config['out-of-seq']:
    print("Running in out-of-seq mode")
  ...
  if change_history_metadata:
    ...
    print("Unable to find change history table %s.%s.%s" % (change_history_table['database_name'], change_history_table['schema_name'], change_history_table['table_name']))
    sys.exit(127)
  ...
  if change_history:
    print('Version records loaded in list =' + str(len(change_history)))
  ...
  for script_name in all_script_names_sorted:
   ...
   # if script_name[0] == 'V' and get_alphanum_key(script['script_version']) <= get_alphanum_key(max_published_version):
    ...
    # If out-of-sequence is turned on, search for files that are out of sequence and process them
    missing = False
    if script_name[0] == 'V' and config['out-of-seq']:
      missing = True
      for ver in change_history:
        if get_alphanum_key(script['script_version']) == get_alphanum_key(ver):
           missing=False
           break
    if missing:
          print('Out of Sequence Version will be processed... ' + script['script_version'])
    else: 
      if script_name[0] == 'V' and get_alphanum_key(script['script_version']) <= get_alphanum_key(max_published_version):
        if config['verbose']:
    ...
    if not config['dry-run']:
      status = apply_change_script(script, content, config['vars'], config['snowflake-database'], change_history_table, snowflake_session_parameters, config['autocommit'], config['verbose'],config['ignore-errors'])
  ...
  print("Successfully applied %d change scripts. Failed to apply %d scripts. Skipped %d scripts." % (scripts_applied, scripts_failed, scripts_skipped))
  if (scripts_failed > 0):
    #file = open("error.txt", "w")
    #file.write(str(scripts_failed) + "\n")
    sys.exit(scripts_failed)
...
def render_command(config, script_path):
  ...
  if not os.path.isfile(script_path):
    ...
    print("Invalid script_path: %s" % script_path)
    sys.exit(127)
...
def get_schemachange_config(config_file_path, root_folder, modules_folder, snowflake_account, snowflake_user, snowflake_role, snowflake_warehouse, snowflake_database, change_history_table_override, vars, create_change_history_table, autocommit, verbose, dry_run, ignore_errors, out_of_seq, query_tag):
  ...
  if not os.path.isdir(config['root-folder']):
    ...
    print("Invalid root folder: %s" % config['root-folder'])
    sys.exit(127)
  ...
  if config['modules-folder']:
    ...
    if not os.path.isdir(config['modules-folder']):
      ...
      print("Invalid modules folder: %s" % config['modules-folder'])
      sys.exit(127)
  ...
  if ignore_errors:
    config['ignore-errors'] = ignore_errors
  if 'ignore-errors' not in config:
    config['ignore-errors'] = False

  if out_of_seq:
    config['out-of-seq'] = out_of_seq
  if 'out-of-seq' not in config:
    config['out-of-seq'] = False
  ...
  if config['vars']:
    ...
    if type(config['vars']) is not dict:
      ...
      print("vars did not parse correctly, please check its configuration")
      sys.exit(127)
    ...
    if "schemachange" in config['vars']:
      ...
      print("The variable schemachange has been reserved for use by schemachange, please use a different nam")
      sys.exit(127)
  ...
  for (directory_path, directory_names, file_names) in os.walk(root_directory):
    for file_name in file_names:
      ...
      if script_name in all_files:
        ...
        print("The script name %s exists more than once (first_instance %s, second instance %s)" % (script_name, all_files[script_name]['script_full_path'], script['script_full_path']))
        sys.exit(127)
      ...
      if script_type == 'V':
        print('Version=' + script['script_version'] )
        if script['script_version'] in all_versions:
         # raise ValueError("The script version %s exists more than once (second instance %s)" % (script['script_version'], script['script_full_path']))
          print("The script version %s exists more than once (second instance %s)" % (script['script_version'], script['script_full_path']))
          sys.exit(127)
...
def execute_snowflake_query(snowflake_database, query, snowflake_session_parameters, autocommit, verbose):
  ...
  #  raise ValueError("Unable to find connection credentials for private key or password authentication")
    print("Unable to find connection credentials for private key or password authentication")
    sys.exit(127)
...
#      raise ValueError("Invalid change history table name: %s" % change_history_table_override)
      print("Invalid change history table name: %s" % change_history_table_override)
      sys.exit(127)
...
def fetch_change_history(change_history_table, snowflake_session_parameters, autocommit, verbose):
  query = "SELECT VERSION FROM {0}.{1} WHERE SCRIPT_TYPE = 'V' AND STATUS='Success' ORDER BY VERSION DESC".format(change_history_table['schema_name'], change_history_table['table_name'])
  results = execute_snowflake_query(change_history_table['database_name'], query, snowflake_session_parameters, autocommit, verbose)
...
def apply_change_script(script, script_content, vars, default_database, change_history_table, snowflake_session_parameters, autocommit, verbose, ignore_errors):
  ...
  if len(script_content) > 0:
    ...
    try:
      ...
      if (ignore_errors) and (script['script_name'][:1].upper()=='R'):
        print('Warning! Errors found in script. Execution will continue')
        status = 'Failed'
        pass
      else:
        print('SQL error encountered in script')
        traceback.print_exc()
        sys.exit(127)
    end = time.time()
...
def main(argv=sys.argv):
  ...
  parser_deploy.add_argument('--ignore-errors', action='store_true', help = 'Run schemachange while ignoring errors (the default is False)', required = False)
  parser_deploy.add_argument('--out-of-seq', action='store_true', help = 'Run schemachange inclulding out-of-sequence V files (the default is False)', required = False)
  ...
  if args.subcommand == 'render':
    config = get_schemachange_config(config_file_path, args.root_folder, args.modules_folder, None, None, None, None, None, None, args.vars, None, None, args.verbose, None, None, None, None)
  else:
    config = get_schemachange_config(config_file_path, args.root_folder, args.modules_folder, args.snowflake_account, args.snowflake_user, args.snowflake_role, args.snowflake_warehouse, args.snowflake_database, args.change_history_table, args.vars, args.create_change_history_table, args.autocommit, args.verbose, args.dry_run, args.ignore_errors, args.out_of_seq, args.query_tag)
